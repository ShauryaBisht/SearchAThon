...
export default function TeamDetails() {
  ...
  useEffect(() => {
    ...
    const handleMemberRemoved = (data: { teamId: string; removedMemberId: string }) => {
      if (data.teamId === id) {
        setTeam((prev) => {
          if (!prev) return null
          return {
            ...prev,
            members: prev.members.filter((m) => m._id !== data.removedMemberId),
          }
        })
      }
    }
    ...
    const handleSelfRemoved = (data: { teamId: string; teamName: string; message: string }) => {
      if (data.teamId === id) {
        alert(data.message || `You were removed from ${data.teamName}`)
        navigate("/teams")
      }
    }

    socket.on("team-member-updated", handleMemberRemoved)
    socket.on("removed-from-team", handleSelfRemoved)

    return () => {
      socket.off("team-member-updated", handleMemberRemoved)
      socket.off("removed-from-team", handleSelfRemoved)
    }
  }, [socket, id, navigate])
    ...
    if (team) {
      ...
      if (acceptedUser) {
        setTeam({
          ...
          members: [
            ...team.members,
            { _id: acceptedUser._id, fullName: acceptedUser.fullName, avatar: acceptedUser.avatar },
          ],
        })
      }
    }
                ...
                >
                  ...
                  {canRemove && (
                    <button
                      ...
                      className="ml-1 p-0.5 text-slate-400 hover:text-red-400 hover:bg-slate-700 rounded transition cursor-pointer"
              ...
              {team.joinRequests.map((reqUser) => (
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      className="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-md text-sm cursor-pointer transition"
                    ...
                    <button
                      type="button"
                      className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded-md text-sm cursor-pointer transition"
